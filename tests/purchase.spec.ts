...
test("Buy products end-to-end", async ({ page }) => {
  ...
  await test.step("Add products to the cart", async () => {
    ...
    for (const { productName, quantity } of testData.productsPurchase) {
      await mainPage.goToProductDetailsPage(productName);
...
//     for (const { id, productName, quantity } of productsPurchase.productsPurchase) {
